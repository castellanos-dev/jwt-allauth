...
class SocialProvidersCheckTests(SimpleTestCase):
    """Routed social endpoints with nothing configured answer 404 to everything."""
    ...
    def test_warns_when_no_app_is_configured(self):
    ...
    @override_settings(ROOT_URLCONF='empty_urls', SOCIALACCOUNT_PROVIDERS={})
    def test_reports_the_missing_extra_when_the_app_is_installed_but_unroutable(self):
        # `empty_urls` stands in for the endpoints not being routed. With the app in
        # INSTALLED_APPS that can only mean the dependencies are missing, which is the
        # state a project generated by `startproject` upgrades into.
        with patch.dict('sys.modules', {'jwt_allauth.social.urls': None}):
            warnings = check_social_providers(None)
        self.assertEqual([w.id for w in warnings], [SOCIAL_NO_PROVIDERS_ID])
