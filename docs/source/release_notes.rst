...
- **Registration no longer discloses which addresses are registered**: signing up with an address that was already taken answered ``400`` with *"A user is already registered with this e-mail address."*, so anybody could walk a list of addresses through the endpoint and learn who has an account. The endpoint now follows allauth's ``ACCOUNT_PREVENT_ENUMERATION`` (enabled by default): while email verification is mandatory, a conflicting address gets the very same ``201`` response as a free one, no account is created, and the owner of the address is notified by email instead of receiving a confirmation link. Two consequences are worth noting: that response no longer carries a ``refresh`` token — it could not be issued for an address belonging to somebody else, and its presence would give the answer away, while it is unusable until the address is verified anyway — and an address that an established account has not confirmed yet counts as taken. Set ``ACCOUNT_PREVENT_ENUMERATION = False`` to restore the previous behaviour, error and refresh token included. With ``EMAIL_VERIFICATION = False`` the conflict is reported as before, since a successful sign-up hands out session tokens that cannot be faked. The admin-managed endpoint is unaffected: administrators are told that an address is in use. The endpoint also honours DRF's ``anon`` throttle rate now.

- **Pending registrations are no longer destroyed by anyone who knows the address**: validating a registration deleted *every* unverified ``EmailAddress`` matching it, before the rest of the payload was even checked. An anonymous request — including one that ended in ``400`` — was enough to invalidate somebody else's confirmation link, or to strip an established account of a secondary address it was still confirming, and the account behind the deleted address was left in place without one. Nothing is deleted during validation any more, and an address is only taken over once the account holding it is confirmed to be a sign-up that nobody ever completed: unverified, never logged into, not privileged and with no verified address of its own. Such a superseded sign-up is now removed as a whole, user row included, instead of accumulating accounts that no longer own their address.
