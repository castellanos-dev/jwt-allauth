...
- **TOTP brute force limited per user, not just per challenge**: the MFA verification step invalidated a login challenge after 5 failed codes, but an attacker holding the password could simply log in again for a fresh challenge and get 5 more guesses, indefinitely. Failed verifications are now also counted per user across every challenge and across both ``/mfa/verify/`` and ``/mfa/verify-recovery/`` (``JWT_ALLAUTH_MFA_MAX_ATTEMPTS``, default 10, over a sliding ``JWT_ALLAUTH_MFA_LOCKOUT_SECONDS`` window, default 900). When the budget is spent, outstanding challenges are dropped, the verification endpoints answer ``429`` with ``Retry-After`` without checking the code, and ``/login/`` refuses to issue a new challenge. The per-challenge limit is now configurable too (``JWT_ALLAUTH_MFA_CHALLENGE_MAX_ATTEMPTS``). Additionally, the attempt bookkeeping used to write the attempt and then count it without a lock, so concurrent requests could slip under the threshold together; the whole read-modify-write now runs in a transaction that locks the user row first.
