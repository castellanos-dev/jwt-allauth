...
- **Refresh token rotation is atomic**: the whitelist was queried, checked, cleared and repopulated without a transaction or a row lock, so two requests presenting the same refresh token simultaneously both passed the whitelist check and both whitelisted a successor. A single stolen credential therefore produced two live sessions, and the replay detection that revokes a session on reuse never fired. The rotation now runs inside a transaction that locks the whitelist entry before reading it and treats the deletion of that entry as the claim, so exactly one of the competing requests can mint the successor and the other is handled as the replay it is. ``jti`` is now unique in the database as a last barrier, which also turns the ``IntegrityError`` guard on the rotation — previously unreachable — into a real one. **A migration is required**: run ``python manage.py makemigrations jwt_allauth`` and ``python manage.py migrate`` after upgrading.

- **Password reset and password set capabilities are consumed atomically**: both endpoints looked the single-use token up, verified it and deleted it afterwards, so two simultaneous requests carrying the same cookie both got through and set a password — with different bodies, the last writer won and the user had no way of knowing which password was kept. The token is now claimed by its deletion, and only the request that actually removed the row proceeds. The same claim is applied when a reset link is exchanged for a capability. In addition, ``PasswordResetConfirmView`` used to mint a capability without invalidating the ones handed out before it, leaving one live capability per reset request; only the most recent one stays valid now.
