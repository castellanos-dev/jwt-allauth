...
- **MFA installs are repaired, and the fido2 cap is gone**: the ``mfa`` extra pinned ``fido2<2.0.0`` next to ``django-allauth[mfa]>=65.5``, and that pair was the wrong way round. allauth up to 65.8 asked for ``fido2>=1.1.2`` with no upper bound of its own, and fido2 2.0 dropped the ``features.webauthn_json_mapping`` flag that allauth sets when its WebAuthn module is imported — so anything resolving allauth 65.5 against a current fido2 raised ``AttributeError`` on every request touching ``allauth.mfa``. The cap hid that for installs of this package while freezing them out of the fido2 line where WebAuthn security fixes land. The floor of the extra is ``django-allauth[mfa]>=65.9`` now — the first allauth that bounds fido2 itself and works with 2.x — and fido2 is left to allauth to constrain. Projects on the ``mfa`` extra move up by four allauth minors; the floor of the core dependency is untouched at 65.5 for projects that do not use MFA. The same pair had been failing the minimum-dependency CI environment silently, 34 MFA tests at a time.
...
- **A security policy and a contributing guide**: ``SECURITY.md`` gives vulnerability reports a private channel — a package that argues for itself on security had no business asking for them in the public issue tracker — and states what is out of scope, including the one exposure this design accepts by name: access tokens outliving a revoked session. ``CONTRIBUTING.md`` covers the tox matrix and the three areas where a change needs to know what it is touching, session bookkeeping first among them.
...
- **Simple JWT is described as the layer it is, not as a competitor**: the comparison first drawn up put Simple JWT in a column of its own with crosses against it, which reads as a package to be chosen instead of this one. It is a hard dependency: its ``RefreshToken`` is subclassed, its ``JWTAuthentication`` is subclassed, and its signing and verification are used untouched. What this library replaces is one part of it — the blacklist that decides when a session ends, refused at startup and substituted by a whitelist keyed by session, which is what turns a replayed token from a rejected request into a revoked session. A *Relationship to Simple JWT* section now states that, and says plainly that a project needing only signed tokens should use Simple JWT directly. The table alongside it compares the packages that do occupy the same slot, djoser included — it depends on Simple JWT outright, where dj-rest-auth defaults to DRF's own tokens and treats JWT as opt-in.
