...
- **Revoking a session now invalidates its access tokens**: revocation removed the refresh tokens from the whitelist, which stopped rotation but left the access tokens already issued for that session working until they expired — up to ``JWT_ALLAUTH_ACCESS_TOKEN_LIFETIME``, 30 minutes by default. Replaying a rotated refresh token was the worst case: the reuse is only detected when the legitimate client rotates next, so it was logged out immediately while the client that replayed the token kept a freshly minted access token. The same gap applied to ``/logout/``, ``/logout-all/``, password changes and resets, the absolute session lifetime and deactivated accounts. Authentication now checks the ``session`` claim of the access token against the whitelist and answers ``401`` with code ``token_not_valid`` when the session is gone. ``jwt_allauth.authentication.JWTAllAuthAuthentication`` replaces simplejwt's ``JWTStatelessUserAuthentication`` as the default authentication class — the user is still built from the token claims, at the cost of one indexed query per request — and ``jwt_allauth.authentication.SessionRevocationMixin`` is available for projects with their own class. Set ``JWT_ALLAUTH_ACCESS_TOKEN_SESSION_CHECK = False`` to keep the previous, fully stateless behaviour. The whitelist ``jti`` and ``session`` columns are now indexed: run ``python manage.py makemigrations jwt_allauth && python manage.py migrate``.
