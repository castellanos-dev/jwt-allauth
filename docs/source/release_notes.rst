...
- **An empty nullable role reads as the account's flags rather than as** ``null``: the one behaviour change of this release that an existing installation can see. ``JAUser`` and ``RoleMixin`` declare the field ``null=False`` with a default, so nothing changes for either; a project that declared a nullable ``role`` of its own, however, had every account with the column empty carrying ``"role": null`` in its tokens, and ``null`` matches no permission class — including the staff and superuser codes :class:`~jwt_allauth.permissions.BasePermission` grants access on top of whatever a class accepts. Those accounts now carry the code their staff flags imply, so a staff member whose role column was never filled in regains the access the permission classes have always documented. Nobody loses access; if that is not wanted, fill the column.
...
- **MFA installs are repaired, and the fido2 cap is gone**: the ``mfa`` extra pinned ``fido2<2.0.0`` next to ``django-allauth[mfa]>=65.5``, and that pair was the wrong way round. allauth up to 65.8 asked for ``fido2>=1.1.2`` with no upper bound of its own, and fido2 2.0 dropped the ``features.webauthn_json_mapping`` flag that allauth sets when its WebAuthn module is imported — so anything resolving allauth 65.5 against a current fido2 raised ``AttributeError`` on every request touching ``allauth.mfa``. The cap hid that for installs of this package while freezing them out of the fido2 line where WebAuthn security fixes land. The floor of the extra is ``django-allauth[mfa]>=65.9`` now — the first allauth that bounds fido2 itself and works with 2.x — and fido2 is left to allauth to constrain. Projects on the ``mfa`` extra move up by four allauth minors; the floor of the core dependency is untouched at 65.5 for projects that do not use MFA.
...
- **The package declares its licence**: ``pyproject.toml`` never declared one, so the published metadata carried no licence at all for a project that has always been MIT — which is what an automated dependency review reads. ``license`` and the OSI classifier are set, and the sdist carries ``License-File: LICENSE``.

Documentation
~~~~~~~~~~~~~

- **A page on refresh token theft**: :doc:`refresh_token_theft` covers why rotating refresh tokens does not detect theft on its own, what a blacklist catches and what it lets through, and what reuse detection has to do instead — including the four ways an implementation of it fails silently: a rotation that is not atomic and turns one credential into two live sessions, a revocation rolled back with the transaction that detected the replay, a revocation racing a rotation and leaving the successor behind, and the absence of an absolute session lifetime for the token nobody ever contests. Written to be worth reading whether or not the reader ends up using this package. :doc:`refresh_token` remains the reference for the settings.

- **A security policy and a contributing guide**: ``SECURITY.md`` gives vulnerability reports a private channel and sets out what is in scope and what is not — including the exposure this design accepts by name, access tokens outliving a revoked session. ``CONTRIBUTING.md`` covers the tox matrix and the areas where a change needs to know what it is touching, session bookkeeping first among them.

- **A comparison against dj-rest-auth, djoser and** ``allauth.headless``, in the README and on the documentation index, covering session handling, the endpoints each ships and the second-factor support of each — including the row this library loses, social authentication, with a pointer to the packages that cover it.

- **Documentation pages no longer carry the release number in their title**, so a search result or a bookmark stays accurate across releases. Canonical URLs, meta descriptions and Open Graph tags are set.
