...
- Modules configuration
    ...
    - ``JWT_ALLAUTH_ACCESS_TOKEN_SESSION_CHECK`` - whether every authenticated request checks that the session behind the access token is still whitelisted (default: ``True``). This is what makes revocation effective immediately instead of when the access token expires, at the cost of one indexed query per request. Set it to ``False`` for fully stateless authentication, accepting that a revoked session stays usable for up to ``JWT_ALLAUTH_ACCESS_TOKEN_LIFETIME``. See :doc:`refresh_token`.
