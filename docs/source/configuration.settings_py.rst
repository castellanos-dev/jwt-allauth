...
- Modules configuration
    ...
    - ``JWT_ALLAUTH_ACCESS_TOKEN_SESSION_CHECK`` - whether every authenticated request verifies that the session behind the access token is still whitelisted (default: ``True``). This is what makes revocation — ``/logout/``, ``/logout-all/``, a password change, an expired session, a deactivated account or a reused refresh token — take effect immediately instead of when the access token expires. It costs one indexed query per authenticated request; set it to ``False`` for fully stateless authentication, accepting that a revoked session stays usable for up to ``JWT_ALLAUTH_ACCESS_TOKEN_LIFETIME``. The check is applied by the authentication classes in ``jwt_allauth.authentication``; see :doc:`refresh_token`.
